    ...
    if cap.isOpened():
        ...
        while cv2.getWindowProperty("CSI Camera " + str(cam), 0) >= 0:
            ...
            if key == ord("s"):
                trigger = 1
                if cam == 1:
                    cv2.imwrite("Result_Images/Camera_A_ %s.jpg" % now, newImg)
                    break
            ...
            if trigger == 1 and cam != 1:
                ...
                if FinalTime >= timeDelay:
                    if cam == 2:
                        cv2.imwrite("Result_Images/Camera_B_ %s.jpg" % now, newImg)
                        break
                    elif cam == 3:
                        cv2.imwrite("Result_Images/Camera_C_ %s.jpg" % now, newImg)
                        break
                    elif cam == 4:
                        cv2.imwrite("Result_Images/Camera_D_ %s.jpg" % now, newImg)
                        break
        cap.release()
