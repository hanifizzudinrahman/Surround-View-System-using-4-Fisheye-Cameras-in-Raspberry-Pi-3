...
# CAR Image (Dimension must [186x229])
Car_Name = "_CAR_.jpg"
#=========================================================
# =============== [ P A R A M E T E R ] - [ F U L L ]
...
def main():
    ...
    while True:
        ...
        # Add Car Image
        imgStitch = addCar(imgStitch, Car_Name, pointXMin,pointXMax,pointYMin,pointYMax)
        # Add Line Behind Car
        imgStitch = addLine(imgStitch)
        # Crop Unimportant Parts and Rescale
        imgStitch = cropParts(imgStitch)
        # Calculate FPS
        FPS = calculateFPS(imgStitch,startTime)
        print(".")
        
        # Show Image Stitch
        cv2.imshow('FINAL', imgStitch)
        # Save Image or Stop
        key = cv2.waitKey(1) & 0xFF
        if key == ord("q") or key == ord("d"):
            done = 1
        elif key == ord("s"):
            saveName = "FINAL (FPS) %s.jpg" % now
...
def stitchCamera(imgA,imgB,imgC,imgD, imgMaskX_Name, imgMaskY_Name):
    ...
    return imgStitching
#>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
def addCar(img, imgCar_Name, xMin,xMax,yMin,yMax):
    imgCar = cv2.imread(imgCar_Name)
...
def cropParts(img):
    ...
    xDistance = 125
    yDistance = 145
    img = img[pointYMin-yDistance : pointYMax+yDistance , pointXMin-xDistance : pointXMax+xDistance]
    # Rescale
    h,w,c = img.shape
    scale = 1.6
